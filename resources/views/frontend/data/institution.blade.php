...
<section>
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <h4>{{ ucwords(strtolower(\TCG\Voyager\Models\User::findOrFail($listOfData[0]->assign)->name)) }}</h4>
            </div>
        </div><br>
    <div class="row">
        <div class="col-lg-8">
            <div id="accordion" class="faq accordion accordion-custom pb-5">
                <div class="card dataset-header">
                    <div id="headingOne">
                        <span>Menampilkan {{ count($listOfData) }} Dataset</span>
                    </div>
                </div>
                <div class="list-group">
                    @foreach ($listOfData as $table)
                        <div class="list-data">
                            <a href="{{ route('show_data_detail', [
                                'id'   => $table->template_id,
                                'year' => $table->year
                            ]) }}" class="data-title">
                                {{ ucwords(strtolower(\App\Models\Template::findOrFail($table->template_id)->name)) }}
                                Tahun {{ $table->year }}
                            </a><br>
                            <small>
                                {{ \TCG\Voyager\Models\User::findOrFail(\App\Models\Template::findOrFail($table->template_id)->assign)->name }},
